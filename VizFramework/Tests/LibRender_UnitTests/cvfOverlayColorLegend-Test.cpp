...
/// 
//--------------------------------------------------------------------------------------------------
TEST(OverlayColorLegendTest, Sizeing)
{
    ref<Font> myFont = new FixedAtlasFont(FixedAtlasFont::STANDARD);
    OverlayColorLegend cl(myFont.p());
    ...
    EXPECT_EQ(500, cl.maximumSize().x());
    EXPECT_EQ(100, cl.maximumSize().y());

    // Minimum size hard coded to 100,100
    EXPECT_EQ(100, cl.minimumSize().x());
    EXPECT_EQ(100, cl.minimumSize().y());
}
